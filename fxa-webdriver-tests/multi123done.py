...
if LOCAL:
    chosen_browsers = [ ('Linux', 'firefox', '29')]
...
btn_logout_indicator = "button.logout"
...
class FxaTest(unittest.TestCase):
    ...
    def _test_123done_signup(self):
        ...
        time.sleep(10)
        # self.wait_page_load(FXA_123DONE)
    ...
    def _test_123done_signin(self):
        '''123done2: signin, changepw, delete  - %s %s %s''' % \
        (self.os, self.br, self.version)
        ...
        signout_link = self.driver.find_element_by_id('logout')
        assert("" in signout_link.get_attribute("innerHTML"))
